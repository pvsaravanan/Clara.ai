...
const Page = () => {
  ...
  const fetchArticles = async (topic: string) => {
    ...
    try {
      ...
      if (res.status === 503) {
        toast.error(data.message || 'SearxNG is not configured');
        setDiscover([]);
        setLoading(false);
        return;
      }
        ...
        <div className="flex flex-col gap-4 pb-28 pt-5 lg:pb-8 w-full">
          {!discover || discover.length === 0 ? (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
              <Globe2Icon size={64} className="mb-4 text-black/30 dark:text-white/30" />
              <h3 className="text-xl font-medium mb-2 text-black/70 dark:text-white/70">
                No articles available
              </h3>
              <p className="text-sm text-black/50 dark:text-white/50 max-w-md">
                SearxNG is not configured. Please go to Settings → Search and configure the SearxNG URL to use the Discover feature.
              </p>
            </div>
          ) : (
            <>
